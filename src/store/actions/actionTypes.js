/**
 *     ______               __      __               ______               __      __                               
 *    /      \             /  |    /  |             /      \             /  |    /  |                              
 *    /$$$$$$  | __    __  _$$ |_   $$ |____        /$$$$$$  |  _______  _$$ |_   $$/   ______   _______    _______ 
 *    $$ |__$$ |/  |  /  |/ $$   |  $$      \       $$ |__$$ | /       |/ $$   |  /  | /      \ /       \  /       |
 *    $$    $$ |$$ |  $$ |$$$$$$/   $$$$$$$  |      $$    $$ |/$$$$$$$/ $$$$$$/   $$ |/$$$$$$  |$$$$$$$  |/$$$$$$$/ 
 *    $$$$$$$$ |$$ |  $$ |  $$ | __ $$ |  $$ |      $$$$$$$$ |$$ |        $$ | __ $$ |$$ |  $$ |$$ |  $$ |$$      \ 
 *    $$ |  $$ |$$ \__$$ |  $$ |/  |$$ |  $$ |      $$ |  $$ |$$ \_____   $$ |/  |$$ |$$ \__$$ |$$ |  $$ | $$$$$$  |
 *    $$ |  $$ |$$    $$/   $$  $$/ $$ |  $$ |      $$ |  $$ |$$       |  $$  $$/ $$ |$$    $$/ $$ |  $$ |/     $$/ 
 *    $$/   $$/  $$$$$$/     $$$$/  $$/   $$/       $$/   $$/  $$$$$$$/    $$$$/  $$/  $$$$$$/  $$/   $$/ $$$$$$$/  
*/
export const INIT_LOGIN = 'INIT_LOGIN';
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS';
export const LOGIN_FAIL = 'LOGIN_FAIL';
export const LOGOUT_SUCCESS = 'LOGOUT_SUCCESS';


/**  
 * 
 *     _______                       __                                            __         ______               __      __                               
 *    /       \                     /  |                                          /  |       /      \             /  |    /  |                              
 *    $$$$$$$  |  ______    _______ $$ |____    ______    ______    ______    ____$$ |      /$$$$$$  |  _______  _$$ |_   $$/   ______   _______    _______ 
 *    $$ |  $$ | /      \  /       |$$      \  /      \  /      \  /      \  /    $$ |      $$ |__$$ | /       |/ $$   |  /  | /      \ /       \  /       |
 *    $$ |  $$ | $$$$$$  |/$$$$$$$/ $$$$$$$  |/$$$$$$  | $$$$$$  |/$$$$$$  |/$$$$$$$ |      $$    $$ |/$$$$$$$/ $$$$$$/   $$ |/$$$$$$  |$$$$$$$  |/$$$$$$$/ 
 *    $$ |  $$ | /    $$ |$$      \ $$ |  $$ |$$ |  $$ | /    $$ |$$ |  $$/ $$ |  $$ |      $$$$$$$$ |$$ |        $$ | __ $$ |$$ |  $$ |$$ |  $$ |$$      \ 
 *    $$ |__$$ |/$$$$$$$ | $$$$$$  |$$ |__$$ |$$ \__$$ |/$$$$$$$ |$$ |      $$ \__$$ |      $$ |  $$ |$$ \_____   $$ |/  |$$ |$$ \__$$ |$$ |  $$ | $$$$$$  |
 *    $$    $$/ $$    $$ |/     $$/ $$    $$/ $$    $$/ $$    $$ |$$ |      $$    $$ |      $$ |  $$ |$$       |  $$  $$/ $$ |$$    $$/ $$ |  $$ |/     $$/ 
 *    $$$$$$$/   $$$$$$$/ $$$$$$$/  $$$$$$$/   $$$$$$/   $$$$$$$/ $$/        $$$$$$$/       $$/   $$/  $$$$$$$/    $$$$/  $$/  $$$$$$/  $$/   $$/ $$$$$$$/  
*/
// TODAY
export const INIT_TODAY_EVENTS = 'INIT_TODAY_EVENTS'
export const TODAY_EVENTS_SUCCESS = 'TODAY_EVENTS_SUCCESS'
export const TODAY_EVENTS_FAIL = 'TODAY_EVENTS_FAIL'
export const SET_TODAY_EVENTS = 'SET_TODAY_EVENTS'

// FILTERED
export const INIT_FILTERED_EVENTS = 'INIT_FILTERED_EVENTS'
export const FILTERED_EVENTS_SUCCES = 'FILTERED_EVENTS_SUCCES'
export const FILTERED_EVENTS_FAIL = 'FILTERED_EVENTS_FAIL'
export const SET_FILTERED_EVENTS = 'SET_FILTERED_EVENTS'

// LATEST EVENTS
export const INIT_LATEST_EVENTS = 'INIT_LATEST_EVENTS'
export const LATEST_EVENTS_SUCCESS = 'LATEST_EVENTS_SUCCESS'
export const LATEST_EVENTS_FAIL = 'LATEST_EVENTS_FAIL'
export const SET_LATEST_EVENTS = 'SET_LATEST_EVENTS'

/**  
 *  ________                                __                      ______               __      __                               
 * /        |                              /  |                    /      \             /  |    /  |                              
 * $$$$$$$$/__     __  ______   _______   _$$ |_    _______       /$$$$$$  |  _______  _$$ |_   $$/   ______   _______    _______ 
 * $$ |__  /  \   /  |/      \ /       \ / $$   |  /       |      $$ |__$$ | /       |/ $$   |  /  | /      \ /       \  /       |
 * $$    | $$  \ /$$//$$$$$$  |$$$$$$$  |$$$$$$/  /$$$$$$$/       $$    $$ |/$$$$$$$/ $$$$$$/   $$ |/$$$$$$  |$$$$$$$  |/$$$$$$$/ 
 * $$$$$/   $$  /$$/ $$    $$ |$$ |  $$ |  $$ | __$$      \       $$$$$$$$ |$$ |        $$ | __ $$ |$$ |  $$ |$$ |  $$ |$$      \ 
 * $$ |_____ $$ $$/  $$$$$$$$/ $$ |  $$ |  $$ |/  |$$$$$$  |      $$ |  $$ |$$ \_____   $$ |/  |$$ |$$ \__$$ |$$ |  $$ | $$$$$$  |
 * $$       | $$$/   $$       |$$ |  $$ |  $$  $$//     $$/       $$ |  $$ |$$       |  $$  $$/ $$ |$$    $$/ $$ |  $$ |/     $$/ 
 * $$$$$$$$/   $/     $$$$$$$/ $$/   $$/    $$$$/ $$$$$$$/        $$/   $$/  $$$$$$$/    $$$$/  $$/  $$$$$$/  $$/   $$/ $$$$$$$/  
*/
export const INIT_EVENTS = 'INIT_EVENTS'
export const EVENTS_SUCCESS = 'EVENTS_SUCCESS'
export const EVENT_DETAIL_SUCCESS = 'EVENT_DETAIL_SUCCESS'
export const INIT_EVENT_DETAIL = 'INIT_EVENT_DETAIL'
export const EVENT_DETAIL_FAIL = 'EVENT_DETAIL_FAIL'
export const EVENTS_FAIL = 'EVENTS_FAIL'
export const SET_EVENTS = 'SET_EVENTS'
export const INIT_UPSERT_EVENT = 'INIT_UPSERT_EVENT'
export const UPSERT_EVENT_SUCCESS = 'UPSERT_EVENT_SUCCESS'
export const UPSERT_EVENT_FAIL = 'UPSERT_EVENT_FAIL'
export const RESTART_EVENT_FORM = 'RESTART_EVENT_FORM'
